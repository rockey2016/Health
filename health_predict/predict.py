...
def transform_data(x,y,is_predict=True):
    if is_predict is True:
        with open('./data/scaler.hdf5', 'rb') as f:
        ...
        x_ = scaler.fit_transform(x)
        y_ = y/100
        with open('./data/scaler.hdf5', 'wb') as f:
...
class LstmModel():
    ...
    def create_model(self):
        ...
        self._model.add(LSTM(128, input_shape=(timesteps_in, dim_in), return_sequences=True))
        self._model.add(Dropout(0.3))
        self._model.add(LSTM(64, return_sequences=True))
        self._model.add(Dropout(0.3))
        self._model.add(LSTM(32, return_sequences=True))
        self._model.add(Dropout(0.3))
        self._model.add(LSTM(16, return_sequences=False))
        self._model.add(Dropout(0.3))
    ...
    def train_model(self, train_x, train_y, valid_x, valid_y, model_save_path):
        ...
        plot_model(self._model, to_file='./models/model_0626.png')
