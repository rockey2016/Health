...
db_dsn_1 = "dbname=%s user=%s password=%s host=%s port=%s" % (
    db_name_1, db_user, db_password, db_host, db_port)
health_check_id = 176
...
sql_get_score = "SELECT * from h_health_check_deduct where health_score >1 and health_check_id=(%s) and (record_time >= '2018-06-01 00:00:00' and record_time < '2018-07-01 00:00:00') order by record_time" % (
    health_check_id)
sql_get_detail = "SELECT * from h_health_check_detail where metric_id is not null and health_check_id=(%s) and (record_time >= '2018-06-01 00:00:00' and record_time < '2018-07-01 00:00:00') order by record_time" % (
    health_check_id)
sql_get_itemscore = "select * from h_health_check_item_score where health_check_id=(%s) and (record_time >= '2018-06-01 00:00:00' and record_time < '2018-07-01 00:00:00') order by record_time" % (
    health_check_id)
...
def get_health_datasets(columns, csv_file, sql_get_score, sql_get_detail, sql_get_itemscore):
    ...
    with psycopg2.connect(db_dsn_1) as conn:
        ...
        for row in score_set:
            main_key = '%s_%d' % (row['record_time'].strftime(
                "%Y%m%d%H%M%S"), row['health_check_id'])
...
if __name__ == '__main__':
    ...
    get_health_datasets(header_health, csv_file_health,
                        sql_get_score, sql_get_detail, sql_get_itemscore)
