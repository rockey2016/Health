...
class LstmModel():
    def __init__(self, model=None, epoches=epoches, batch_size=batch_size, lr=adam_lr, units=units, dr=dr,
        ...
        print ("LSTM model unit1_size:{0} unit2_size:{1} unit3_size:{2} unit4_size:{3} unit5_size:{4} \n".format(
            self._units['ls_units1'],self._units['ls_units2'],self._units['ls_units3'],
            self._units['ls_units4'],self._units['ds_units5']))
        
    def create_model(self):
        self._model = Sequential()
        self._model.add(LSTM(int(self._units['ls_units1']), input_shape=(timesteps_in, dim_in), return_sequences=True))
        self._model.add(Dropout(self._dr))
        self._model.add(BatchNormalization())
        #self._model.add(LSTM(self._units['lstm_units']['units2'], return_sequences=True))
        #self._model.add(Dropout(self._dr))
        #self._model.add(BatchNormalization())
        self._model.add(LSTM(int(self._units['ls_units3']), return_sequences=True))
        self._model.add(Dropout(self._dr))
        self._model.add(BatchNormalization())
        self._model.add(LSTM(int(self._units['ls_units4']), return_sequences=False))
        self._model.add(Dropout(self._dr))
        self._model.add(BatchNormalization())
        ...
        self.logv("LSTM model unit1_size:{0} unit2_size:{1} unit3_size:{2} unit4_size:{3} unit5_size:{4}\n".format(
            self._units['ls_units1'],self._units['ls_units2'],self._units['ls_units3'],
            self._units['ls_units4'],self._units['ds_units5']))'''
